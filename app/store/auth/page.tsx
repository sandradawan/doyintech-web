...
export default function StoreAuthPage() {
  ...
  const field =
    "w-full rounded-xl border border-white/12 bg-[#0a0f1a] px-3.5 py-3 text-sm text-white outline-none transition focus:border-[#ff8c14]/60 focus:ring-1 focus:ring-[#ff8c14]/30";
      ...
      <main className="min-h-screen bg-[#070b12] pt-24 pb-24">
        <div className="mx-auto max-w-[480px] px-6">
          <div className="mb-8 text-center">
            <Link
              href="/store"
              className="text-[12px] font-semibold uppercase tracking-[0.1em] text-[#ff8c14]"
            >
              DoyinStore
            </Link>
            ...
            <p className="mt-2 text-[14px] leading-relaxed text-[#94a3b8]">
              {mode === "login"
                ? "Developers and admins use the same sign-in. Role is detected from your email."
                : "Free registration. An admin must approve your account before you can publish apps."}
            </p>
          </div>
          ...
          >
            {mode === "register" && (
              <>
                <label className="block text-[12px] font-medium text-[#94a3b8]">
                  ...
                  <input name="displayName" required className={`mt-1.5 ${field}`} />
                </label>
                <label className="block text-[12px] font-medium text-[#94a3b8]">
                  Support email *
                  <input name="email" type="email" required className={`mt-1.5 ${field}`} />
                </label>
                <label className="block text-[12px] font-medium text-[#94a3b8]">
                  Website / portfolio
                  <input
                ...
                <label className="block text-[12px] font-medium text-[#94a3b8]">
                  Company (optional)
                  <input name="companyName" className={`mt-1.5 ${field}`} />
                </label>
            ...
            {mode === "login" && (
              <>
                ...
                <button
                  ...
                  className="text-[12px] text-[#64748b] underline-offset-2 hover:text-[#94a3b8] hover:underline"
                >
                  {showAdminKey ? "Hide admin access" : "Admin access (staff only)"}
                </button>
                {showAdminKey && (
                  <label className="block text-[12px] font-medium text-[#94a3b8]">
                    Admin key
            ...
            <button
              ...
              className="w-full rounded-full bg-[#ff8c14] py-3.5 text-[15px] font-semibold text-black transition hover:bg-[#ffa03a] disabled:opacity-60"
            >
              {status === "loading"
                ? "Please wait…"
                : mode === "login"
