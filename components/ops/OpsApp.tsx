...
export default function OpsApp() {
  ...
  const dueFollowUps = useMemo(() => {
    ...
    return ws.deals.filter(
      (d) => d.nextFollowUp && d.nextFollowUp <= today && d.stage !== "paid" && d.stage !== "lost"
    );
  }, [ws]);
  ...
  const contacts = !q.trim()
    ...
    : data.contacts.filter((c) =>
        [c.name, c.business, c.phone, c.email, c.notes].filter(Boolean).join(" ").toLowerCase().includes(q.trim().toLowerCase())
      );
  ...
  return (
    <div className="mx-auto flex min-h-[calc(100vh-3rem)] max-w-[1400px]">
      {printInv && (
        <InvoicePrint orgName={data.orgName} profile={data.profile} invoice={printInv} client={byId(printInv.contactId)} onClose={() => setPrintInv(null)} />
      )}
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-[240px] flex-col border-r border-white/[0.06] bg-[#0a0f1a] pt-12 transition-transform lg:static lg:translate-x-0 lg:pt-0 ${menu ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex items-center gap-3 border-b border-white/[0.06] px-4 py-4">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-white">DoyinOps</p>
        ...
        <nav className="flex-1 space-y-0.5 p-2">
          {NAV.map((item) => (
            <button key={item.id} type="button" onClick={() => go(item.id)}
              className={`flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-left text-[13px] font-medium ${tab === item.id ? "bg-[#ff8c14]/15 text-[#ff8c14]" : "text-white/65 hover:bg-white/5"}`}>
              {item.label}
              {item.id === "tasks" && openTasks.length > 0 && (
                <span className="rounded-full bg-[#ff8c14]/20 px-2 text-[10px] text-[#ff8c14]">{openTasks.length}</span>
              )}
            </button>
          ))}
        </nav>
        <div className="space-y-2 border-t border-white/[0.06] p-3">
          ...
          <Link href="/" className="block text-center text-[11px] text-white/30">← Site</Link>
        </div>
      </aside>
                        ...
                        return (
                          <li key={d.id} className="flex justify-between gap-2 rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-2">
                            <div>
                              ...
                              <p className="text-[11px] text-white/45">{nameOf(d.contactId)} · {d.nextFollowUp}</p>
                            </div>
          ...
          {tab === "settings" && (
            <div className="mx-auto max-w-lg space-y-6">
              <form onSubmit={(e: FormEvent<HTMLFormElement>) => {
                ...
                <p className="text-[12px] text-white/45">Shown on printed invoices.</p>
                ...
                <input name="website" defaultValue={data.profile.website || ""} placeholder="Website" className={field} />
                <textarea name="bankNote" defaultValue={data.profile.bankNote || ""} rows={2} placeholder="Bank / payment note" className={field} />
