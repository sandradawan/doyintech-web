...
export function formatEbookHtml(book: Ebook): string {
    ...
    .map((ch) => {
      const img = ch.image
        ? [
            ...
            "<img src='" +
              esc(ch.image) +
              "' alt='' style='width:100%;max-height:280px;object-fit:cover;border-radius:12px'/>",
            ch.imageCaption
      ...
      return [
        "<section style='page-break-inside:avoid;margin:28px 0;padding:20px;border:1px solid #e5e7eb;border-radius:14px;background:#fff'>",
        "<h2 style='font-size:18px;margin:0 0 12px;color:#111'>" + esc(ch.title) + "</h2>",
        img,
        "<div style='white-space:pre-wrap;line-height:1.7;color:#374151;font-size:15px'>" +
          esc(ch.body) +
          "</div>",
        "</section>",
      ].join("");
  ...
  const benefits = book.benefits.map((b) => "<li>" + esc(b) + "</li>").join("");
  ...
  return [
    ...
    "<style>@media print { body { background:#fff!important } .no-print { display:none!important } section { break-inside:avoid } }</style>",
    "</head>",
    ...
    "<img src='" +
      esc(book.coverImage) +
      "' alt='' style='width:100%;height:280px;object-fit:cover;display:block'/>",
    ...
    "<p style='margin:12px 0 0;font-size:13px;opacity:.75'>By " +
      esc(book.author) +
      " · " +
      esc(book.pagesLabel) +
      "</p>",
    ...
    "<ul style='line-height:1.6'>" + benefits + "</ul>",
    "<hr style='border:none;border-top:1px solid #e5e7eb;margin:28px 0'/>",
    chapters,
    "<footer style='margin-top:40px;font-size:12px;color:#9ca3af'>",
    "(c) DoyinTech · Personal use after purchase · https://doyintech.vercel.app/ebooks<br/>",
    ...
    "Tip: Use your browser Print then Save as PDF for a portable PDF copy.",
    "</p>",
    "</body></html>",
  ].join("\n");
}
...
export function formatEbookDocument(book: Ebook): string {
  ...
  lines.push(
    ...
    "Support: doyintechnology@outlook.com | WhatsApp +234 808 534 3926",
    "(c) DoyinTech - for personal use of the purchaser."
  );
